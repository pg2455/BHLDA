...
const unsigned short int K = 3; // Number of topics
...
const unsigned int NUMBER_ITERATIONS = 50; // Number of iterations
...
{
  // Constructor
  topic():d_num_words(0),d_word_topic_allocations(1900000,0){};

  // Member variables
  unsigned long long int d_num_words; // Total number of words allocated to the topic
  std::vector<unsigned long long int> d_word_topic_allocations; // Number of allocations of each word in topic
};
...
void populateTopicsTextBox(const std::unordered_set<unsigned int> &a_vocabulary, topic (&a_topic_array)[K], const text_box &a_text_box)
{
  ...
  //std::cout<<"Begun for loop"<<std::endl;
...
{
  // // Reserve space for topics
  // for (int i =0; i!=K;++i)
  //   { a_body_topic_array[i].d_word_topic_allocations.reserve(a_body_vocabulary.size()+a_headline_vocabulary.size()); } // Make the map the size of the vocabulary
  // for (int i =0; i!=K;++i)
  //   { a_headline_topic_array[i].d_word_topic_allocations.reserve(a_body_vocabulary.size()+a_headline_vocabulary.size()); } // Make the map the size of the vocabulary


  //std::cout<<"Looping through documents"<<std::endl;
  ...
  for (std::list<document>::const_iterator it = a_corpus.cbegin(); it!= a_corpus.cend(); ++it)
    {
      populateTopicsTextBox(a_body_vocabulary, a_body_topic_array,it->d_body); // Populate the topics for the headline
      //std::cout<<"Survived body"<<std::endl;
      populateTopicsTextBox(a_headline_vocabulary, a_headline_topic_array,it->d_headline); // Populate the topics for the headline
      //std::cout<<"Survived headline"<<std::endl;
    }
}
...
void updateAllocation(const unsigned int &a_word, unsigned short int &a_allocation, text_box &a_text_box, unsigned short int (&a_other_topic_allocations)[K], bool a_body_or_not, topic (&a_topic_array)[K], const float &a_VBETA)
{
  ...
  --a_topic_array[a_allocation].d_word_topic_allocations[a_word]; // TODO If it is zero then remove from the vector
...
// bool wayToSort(std::pair<unsigned long long int> i, std::pair<unsigned long long int> j)
...
// 	  std::vector< std::pair<unsigned long long int> > l_sorted_vector; // Vector to store pair values and then to sort them
// 	  l_sorted_vector.reserve(a_topic_array[i].d_num_words); // Reserve the space in memory
// 	  for (std::vector<unsigned long long int>::const_iterator it = a_topic_array[i].d_word_topic_allocations.cbegin();it!=a_topic_array[i].d_word_topic_allocations.cend();++it) // Loop through topic distribution
...
// 	  std::vector< std::pair<unsigned long long int> >::iterator end_it = l_sorted_vector.end();//l_sorted_vector.begin()+NUM_WORDS_DISPLAY_PER_TOPIC;
// 	  for (std::vector< std::pair<unsigned long long int> >::iterator it = l_sorted_vector.begin(); it!= end_it; ++it) // Loop through vector
// 	    {
...
{
  double l_sum = 0;
  for (std::vector<unsigned long long int>::iterator it = a_topic.d_word_topic_allocations.begin();it!= a_topic.d_word_topic_allocations.end(); ++it)
    { l_sum += ( (*it==0) ? LOG_PROB_BIG_CONST : (BETA-1)*log(*it));} // Add the probability value. If the argument is zero then add large constant so that it doesn't blow up to infinity.
  return l_sum;
}
...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  // for (std::vector<unsigned long long int>::iterator it = l_topic1.d_word_topic_allocations.begin();it!=l_topic1.d_word_topic_allocations.end();++it)
  //   {
